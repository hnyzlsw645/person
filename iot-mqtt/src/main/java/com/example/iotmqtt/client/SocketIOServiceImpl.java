package com.example.iotmqtt.client;
    ...
    *//**
    ...
    public void start() {
        ...
        socketIOServer.addEventListener(PUSH_EVENT, PushMessage.class, (client, data, ackSender) -> {
            // TODO do something
            System.err.println("监听中");
            System.err.println(client.toString());
            System.err.println(data);
            System.err.println(ackSender);
    ...
    public void pushMessageToAll(PushMessage pushMessage) {
        if (CollectionUtils.isNotEmpty(clientMap)) {
            clientMap.values().forEach(item -> {
                item.sendEvent(PUSH_EVENT, pushMessage);
            });
        }
    }
